...
setup:
	virtualenv -p `which python3.4` $(WORKON_HOME)/ansible0
	$(WORKON_HOME)/ansible0/bin/pip install -U pip wheel
	$(WORKON_HOME)/ansible0/bin/pip install -Ur requirements/dev.txt
	npm install
	npm update
	cp ansible0/settings/local.example.py ansible0/settings/local.py
	echo "DJANGO_SETTINGS_MODULE=ansible0.settings.local" > .env
	createdb -E UTF-8 ansible0
	$(WORKON_HOME)/ansible0/bin/python manage.py migrate
	if [ -e project.travis.yml ] ; then mv project.travis.yml .travis.yml; fi
	@echo
	@echo "The ansible0 project is now setup on your machine."
	@echo "Run the following commands to activate the virtual environment and run the"
	@echo "development server:"
	@echo
	@echo "	workon ansible0"
	@echo "	npm run dev"

update:
	$(WORKON_HOME)/ansible0/bin/pip install -U -r requirements/dev.txt
	npm install
	npm update
